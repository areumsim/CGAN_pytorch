...
for epoch in range(num_epoch):
    for i, (images, label) in enumerate(data_loader):
        ...
        if (i + 1) % 150 == 0:
            print(
                "Epoch [ {}/{} ]  Step [ {}/{} ]  d_loss : {:.5f}  g_loss : {:.5f}".format(
                    epoch + 1,
                    num_epoch,
                    i + 1,
                    len(data_loader),
    ...
    print(
        " Epock {}'s discriminator performance : {:.2f}  generator performance : {:.2f}".format(
            epoch + 1, d_performance, g_performance
        )
    )
    ...
    save_image(
        samples, os.path.join(dir_name, "CGAN_fake_samples{}.png".format(epoch + 1))
    )
    ...
    fake_sample_image = Image.open(
        "{}/CGAN_fake_samples{}.png".format(dir_name, epoch + 1)
    )
